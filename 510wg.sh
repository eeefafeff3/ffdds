...
function installQuestions() {
	...
	until [[ ${SERVER_WG_NIC} =~ ^[a-zA-Z0ligu9_]+$ && ${#SERVER_WG_NIC} -lt 16 ]]; do
		read -rp "WireGuard interface name: " -e -i wg$(tr -dc 'a-z0-9' </dev/urandom | head -c4) SERVER_WG_NIC
	done

	# 生成随机第三段和第四段（范围 11-222）
	THIRD=$((RANDOM % 212 + 11))  # 随机数范围 11 到 222
...
function newClient() {
	...
	echo "客户端配置"
	echo ""
	echo "客户端名称必须由字母数字字符组成，可包含下划线或破折号，且长度不超过15个字符。"

	until [[ ${CLIENT_NAME} =~ ^[a-zA-Z0-9_-]+$ && ${CLIENT_EXISTS} == '0' && ${#CLIENT_NAME} -lt 16 ]]; do
		CLIENT_NAME="QQQ_$(tr -dc A-Za-z0-9 </dev/urandom | head -c 5)"
		echo "生成的客户端名称: $CLIENT_NAME"

		CLIENT_EXISTS=$(grep -c -E "^### Client ${CLIENT_NAME}\$" "/etc/wireguard/${SERVER_WG_NIC}.conf")

		if [[ ${CLIENT_EXISTS} != 0 ]]; then
			echo ""
			echo -e "${ORANGE}已存在同名客户端，请选择其他名称。${NC}"
			echo ""
		fi
	done
	...
	if [[ ${DOT_EXISTS} == '1' ]]; then
		echo ""
		echo "配置的子网仅支持253个客户端。"
		exit 1
	fi

	BASE_IP=$(echo "$SERVER_WG_IPV4" | awk -F '.' '{ print $1"."$2"."$3 }')
	CLIENT_WG_IPV4="${BASE_IP}.${DOT_IP}"
	echo "分配的客户端 WireGuard IPv4: ${CLIENT_WG_IPV4}"

	BASE_IP=$(echo "$SERVER_WG_IPV6" | awk -F '::' '{ print $1 }')
	CLIENT_WG_IPV6="${BASE_IP}::${DOT_IP}"
	echo "分配的客户端 WireGuard IPv6: ${CLIENT_WG_IPV6}"
